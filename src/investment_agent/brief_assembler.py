...
class _TaggedTheme:
    theme: AgentTheme
    agent: str  # macro | news | equity | quant (brief compat labels)
...
def _collect_tagged(
    ...
    for th in regime.themes:
        out.append(_TaggedTheme(th, "macro"))
    for th in narrative.themes:
        out.append(_TaggedTheme(th, "news"))
    for th in markets.equity_themes:
        out.append(_TaggedTheme(th, "equity"))
    for th in markets.quant_themes:
        out.append(_TaggedTheme(th, "quant"))
    return out
...
def _narrative_sourced_for_cluster(
    ...
    """Attach narrative sourced items only when this cluster includes news themes."""
    news_in_cluster = [t for t in tagged if t.agent == "news"]
    if not news_in_cluster:
        return [], []

    anchor = " ".join(
        f"{t.theme.name} {t.theme.thesis}" for t in news_in_cluster
    )
...
def assemble(
    ...
    *,
    ...
    executive = (
        f"{regime.macro_backdrop} "
        f"Headline tone: {narrative.narrative_sentiment}. {narrative.news_backdrop} "
        f"Top themes: {top_names}."
    ).strip()

    macro_view = (
        f"{regime.macro_backdrop} Dominant regime: {regime.dominant_regime}. "
        + " ".join(regime.cross_asset_signals[:3])
    ).strip()
    news_view = narrative.news_backdrop
    equity_view = markets.equity_view or (
        f"Market style: {markets.market_style}. " + " ".join(markets.valuation_notes[:3])
    )
    quant_view = markets.quant_view or (
    ...
    return InvestmentBrief(
        ...
        macro_view=macro_view[:2000],
        news_view=news_view[:2000],
        equity_view=equity_view[:2000],
        quant_view=quant_view[:2000],
