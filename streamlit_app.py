...
def _render_portfolio(brief: InvestmentBrief | None = None) -> None:
    ...
    view = st.radio(
        "Portfolio view",
        [MANUAL_VIEW_LABEL, MODEL_VIEW_LABEL, COMPARE_VIEW_LABEL],
        horizontal=True,
        key="portfolio_sub_view",
    )

    if view == COMPARE_VIEW_LABEL:
        compare_portfolio_views.render(brief)
    elif view == MODEL_VIEW_LABEL:
        model_portfolio_views.render(brief)
    else:
        manual_portfolio_views.render(brief)
